...
- [x] Add `GuardEvent` model in Rust runtime/service layer.
- [x] Add Rust event publisher trait.
- [x] Add `NoopGuardEventPublisher`.
- [x] Add `JsonlGuardEventPublisher`.
- [x] Emit GuardEvent asynchronously or best-effort after `Check` decision is produced.
- [x] Ensure event publishing failure does not change `Check` response.
- [x] Add Rust service tests for decision independence from event publisher failure.
- [x] Add Rust integration test that writes JSONL event without raw payload.
- [x] Keep Spring `/guard/check` unchanged except documentation/plan role clarification.
- [x] Keep provider relay/proxy out of Stage 10.

### Out of Scope for Stage 10

- [x] Rust HTTP provider proxy remains deferred.
- [x] Actual OpenAI/Anthropic/Gemini relay remains deferred.
- [x] Spring JSONL tailer remains deferred to `monitoring/jsonl-event-tailer`.
- [x] Spring HTTP event collector remains deferred.
- [x] Distributed broker integration remains deferred.
- [x] RDB persistence remains deferred.
- [x] File/archive guard remains deferred.
- [x] Action review module remains deferred.

### Verification

- [x] Rust `cargo test` passes.
- [x] Kotlin `./gradlew test integrationTest` passes.
- [x] Rust `Check` response is returned even when event publisher fails.
- [x] Rust JSONL GuardEvent does not contain raw payload.
- [x] Spring compatibility `/guard/check` remains functional.
- [x] Existing SSE dashboard remains functional for Spring-originated events.

### Merge Gate

- Stage 10 구현은 `main`에 merge 완료.
...
- [x] Rust runtime public check endpoint를 구현한다.
- [x] Rust runtime에서 GuardEvent를 비동기로 생성한다.
- [x] Stage 10 event delivery MVP는 Rust JSONL GuardEvent publisher까지 구현한다.
- [x] Spring 장애 시 Rust runtime decision path가 계속 동작하는지 검증한다.
- [x] Kotlin `/guard/check`와 Rust direct path의 compatibility 기준을 계획에 문서화한다.
- [x] Runtime path gRPC service integration tests를 추가한다.

### Follow-up

- [x] Spring JSONL tailer로 Rust JSONL GuardEvent를 읽어 SSE dashboard에 연결한다.
- [x] Runtime direct path server-level E2E 테스트를 추가한다.
